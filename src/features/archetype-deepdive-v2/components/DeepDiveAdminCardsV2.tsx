...
export interface DeepDiveAdminComparisonV2 {
  assessmentTop3: string[];
  deepDiveTop3: string[];
  overlapCount: number;
  assessmentSummary?: string;
  assessmentStrengths: string[];
  assessmentWatchouts: string[];
  deepDiveHypothesis: string;
  deepDivePrimaryShadow: string;
  adminContractDelta: string[];
}
...
export function DeepDiveAdminCardsV2({
  profile,
  comparisonV2,
}: {
  profile: SampleProfile;
  comparisonV2?: DeepDiveAdminComparisonV2 | null;
}) {
  const { locale } = useLanguage();
  const isFR = locale === "fr";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {comparisonV2 && (
        <Card className="neural-card p-6 sm:p-8 backdrop-blur-3xl bg-white/[0.04] border border-accent-warning/25 shadow-[0_0_50px_-30px_rgba(245,158,11,0.4)]">
          <div className="flex items-center gap-2 text-text-tertiary text-xs uppercase tracking-[0.2em] font-display mb-3">
            <Stethoscope size={14} strokeWidth={1.5} />
            {isFR ? "Diagnostic comparatif V2" : "Comparative diagnostic V2"}
          </div>

          <div className="grid sm:grid-cols-3 gap-4 mb-5">
            <div className="rounded-lg bg-white/[0.02] border border-white/5 p-4">
              <div className="text-[10px] uppercase tracking-[0.25em] text-text-tertiary font-display mb-2">Assessment</div>
              <p className="text-sm text-text-secondary leading-relaxed">
                {comparisonV2.assessmentTop3.join(" – ") || "—"}
              </p>
            </div>
            <div className="rounded-lg bg-white/[0.02] border border-white/5 p-4">
              <div className="text-[10px] uppercase tracking-[0.25em] text-text-tertiary font-display mb-2">Deep Dive</div>
              <p className="text-sm text-text-secondary leading-relaxed">
                {comparisonV2.deepDiveTop3.join(" – ") || "—"}
              </p>
            </div>
            <div className="rounded-lg bg-white/[0.02] border border-white/5 p-4">
              <div className="text-[10px] uppercase tracking-[0.25em] text-text-tertiary font-display mb-2">
                {isFR ? "Recouvrement" : "Overlap"}
              </div>
              <p className="text-sm text-text-secondary leading-relaxed">
                {comparisonV2.overlapCount}/3
              </p>
            </div>
          </div>

          <div className="rounded-lg bg-white/[0.03] border border-white/10 p-4 mb-4">
            <div className="text-[10px] uppercase tracking-[0.25em] text-accent-warning/80 font-display mb-1">
              {isFR ? "Hypothèse admin V2" : "Admin hypothesis V2"}
            </div>
            <p className="text-sm text-text-secondary leading-relaxed">{comparisonV2.deepDiveHypothesis}</p>
          </div>

          <div className="grid md:grid-cols-2 gap-3 mb-4">
            <div className="rounded-lg bg-white/[0.02] border border-white/5 p-4">
              <div className="text-[10px] uppercase tracking-[0.25em] text-text-tertiary font-display mb-2">
                {isFR ? "Forces stables (assessment)" : "Stable strengths (assessment)"}
              </div>
              <ul className="space-y-1">
                {comparisonV2.assessmentStrengths.slice(0, 3).map((s) => (
                  <li key={s} className="text-sm text-text-secondary leading-relaxed">• {s}</li>
                ))}
              </ul>
            </div>
            <div className="rounded-lg bg-white/[0.02] border border-white/5 p-4">
              <div className="text-[10px] uppercase tracking-[0.25em] text-text-tertiary font-display mb-2">
                {isFR ? "Vigilances prioritaires" : "Priority watchouts"}
              </div>
              <ul className="space-y-1">
                {comparisonV2.assessmentWatchouts.slice(0, 2).map((w) => (
                  <li key={w} className="text-sm text-text-secondary leading-relaxed">• {w}</li>
                ))}
                <li className="text-sm text-text-secondary leading-relaxed">
                  • {isFR ? `Ombre principale Deep Dive: ${comparisonV2.deepDivePrimaryShadow}` : `Deep Dive primary shadow: ${comparisonV2.deepDivePrimaryShadow}`}
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="rounded-lg bg-white/[0.02] border border-white/5 p-4">
            <div className="text-[10px] uppercase tracking-[0.25em] text-text-tertiary font-display mb-2">
              {isFR ? "Lecture contrat V2" : "Contract reading V2"}
            </div>
            <ul className="space-y-2">
              {comparisonV2.adminContractDelta.slice(0, 3).map((c, i) => (
                <li key={c} className="text-sm text-text-secondary leading-relaxed flex gap-2">
