...
export default function HabitTracker() {
  ...
  const loadData = async () => {
    ...
    // Load assigned habits + their templates
    ...
    const habitsData: AssignedHabit[] = (assigned as any[]).map((a) => {
      ...
      return {
        ...
        template_category: t?.category || "General",
      };
    });
  ...
  const toggleComplete = async (habitId: string) => {
    ...
    if (isCompleted) {
      ...
      const { error } = await supabase.from("habit_completions" as any).insert({
        user_id: user.id,
    ...
    <div className="space-y-10 max-w-4xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="ethereal-glass p-6">
          ...
          <p className="text-neural-label mt-1">Completed Today</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="ethereal-glass p-6">
          ...
          <p className="text-neural-label mt-1">Assigned Habits</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="ethereal-glass p-6">
          ...
          <p className="text-neural-label mt-1">Today's Score</p>
        </motion.div>
      </div>
      ...
      {habits.length === 0 ? (
        <div className="ethereal-glass p-12 text-center">
          ...
          <p className="text-muted-foreground text-sm">No habits assigned yet. Your coach will assign habits from the admin panel.</p>
        </div>
      ) : (
              ...
              >
                <button
                  ...
                  className={`w-8 h-8 rounded-xl border flex items-center justify-center shrink-0 transition-all duration-300 ${
                    completed ? "bg-primary/20 border-primary/40" : "border-border hover:border-primary/30"
                  }`}
                >
                  {completed && <Check size={14} className="text-primary" />}
                </button>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium transition-colors ${completed ? "line-through text-muted-foreground" : "text-foreground"}`}>
                    {habit.template_name}
                  </p>
