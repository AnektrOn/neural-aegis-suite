    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="ethereal-glass p-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <input type="text" value={newCountry} onChange={(e) => setNewCountry(e.target.value)} placeholder="Pays"
          ...
          <button onClick={addCompany} className="px-4 py-2.5 rounded-xl bg-neural-accent/10 border border-neural-accent/20 text-neural-accent text-sm hover:bg-neural-accent/20 transition-colors">
            <Plus size={16} />
          </button>
        </div>
      </motion.div>
      ...
      <div className="space-y-3">
        ...
        {companies.map((c, i) => (
          <motion.div key={c.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="ethereal-glass p-5 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-neural-label flex items-center gap-1 mt-0.5"><Globe size={10} /> {c.country || "—"}</p>
            </div>
            <button onClick={() => deleteCompany(c.id)} className="p-2 rounded-lg border border-border/30 text-muted-foreground hover:text-destructive hover:border-destructive/30 transition-colors">
              <Trash2 size={14} />
            </button>
          </motion.div>
        ))}
