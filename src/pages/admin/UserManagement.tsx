...
export default function UserManagement() {
  ...
  const toggleDisabled = async (userId: string, isCurrentlyDisabled: boolean) => {
    const { error } = await supabase
      .from("profiles")
      .update({ is_disabled: !isCurrentlyDisabled } as any)
      .eq("id", userId);
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
      toast({ title: isCurrentlyDisabled ? "User Enabled" : "User Disabled" });
      loadUsers();
    }
  };
  ...
  const getCompanyName = (cid: string | null) => {
    if (!cid) return null;
    return companies.find((c) => c.id === cid)?.name || null;
  };
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {userData.is_disabled && (
                    <span className="text-[8px] uppercase tracking-[0.3em] px-2 py-0.5 rounded-full bg-destructive/10 text-destructive border border-destructive/20">
                      Disabled
                    </span>
                  )}
                </div>
                <p className="text-neural-label mt-0.5">
                  {getCompanyName(userData.company_id) || "No company"} · {userData.country || "—"} · Joined {new Date(userData.created_at).toLocaleDateString()}
                </p>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => toggleDisabled(userData.id, userData.is_disabled)}
                  className={`p-2 rounded-lg border transition-all ${
                    userData.is_disabled
                      ? "border-destructive/30 text-destructive"
                      : "border-border/30 text-muted-foreground hover:border-destructive/30 hover:text-destructive"
                  }`}
                  title={userData.is_disabled ? "Enable user" : "Disable user"}
                >
            ...
            {expandedUser === userData.id && (
              <motion.div
                ...
                className="mt-4 pt-4 border-t border-border/10 space-y-4"
              >
                {/* Company assignment */}
                <div>
                  <p className="text-neural-label mb-2">Company</p>
                  <select
                    value={userData.company_id || ""}
                    onChange={(e) => assignCompany(userData.id, e.target.value || null)}
