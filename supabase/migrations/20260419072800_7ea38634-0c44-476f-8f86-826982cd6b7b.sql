...
COMMENT ON TABLE public.place_tag_definitions IS 'Tags sémantiques pour lieux — assignables par admin sur les lieux utilisateurs.';
ALTER TABLE public.place_tag_definitions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users read active place tags" ON public.place_tag_definitions
FOR SELECT TO authenticated USING (active = true);
CREATE POLICY "Admins read all place tags" ON public.place_tag_definitions
FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins insert place tags" ON public.place_tag_definitions
FOR INSERT TO authenticated WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins update place tags" ON public.place_tag_definitions
FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin'))
WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins delete place tags" ON public.place_tag_definitions
...
COMMENT ON TABLE public.user_location_admin_consent IS 'Consentement partage lieux admin.';
ALTER TABLE public.user_location_admin_consent ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users manage own location consent" ON public.user_location_admin_consent
FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Admins read location consent" ON public.user_location_admin_consent
FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users manage own places" ON public.user_places
FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Admins read user places when sharing enabled" ON public.user_places
...
CREATE POLICY "Users read tags on own places" ON public.user_place_tag_assignments
FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM public.user_places p WHERE p.id = user_place_id AND p.user_id = auth.uid())
);
CREATE POLICY "Admins read tags on shared places" ON public.user_place_tag_assignments
FOR SELECT TO authenticated USING (
  public.has_role(auth.uid(), 'admin')
  AND EXISTS (
...
CREATE POLICY "Admins manage tags on shared places" ON public.user_place_tag_assignments
...
CREATE POLICY "Users manage own place contact links" ON public.user_place_contact_links
FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Admins read place contact links when sharing" ON public.user_place_contact_links
FOR SELECT TO authenticated USING (
  public.has_role(auth.uid(), 'admin')
  AND EXISTS (
    SELECT 1 FROM public.user_location_admin_consent c
