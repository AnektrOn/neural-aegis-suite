...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _pulse_admin_filtered (
    user_id UUID,
    user_name TEXT,
    assimilated INT,
    ignored INT,
  ...
  DELETE FROM _pulse_admin_filtered;

  INSERT INTO _pulse_admin_filtered
  WITH swipe_agg AS (
    SELECT
      i.user_id,
  ...
  SELECT
    pr.id,
    COALESCE(NULLIF(TRIM(pr.display_name), ''), pr.id::text),
    COALESCE(sa.assimilated, 0)::int,
    COALESCE(sa.ignored, 0)::int,
    COALESCE(sa.completed, 0)::int,
    COALESCE(sa.total_swipes, 0)::int,
    COALESCE(ra.runes_unlocked, 0)::int,
    sa.last_swipe_at
  ...
  WHERE (
    ...
    AND (p_card_id IS NULL OR sa.user_id IS NOT NULL);

  SELECT COUNT(*) INTO _total FROM _pulse_admin_filtered;

  SELECT COALESCE(jsonb_agg(row_to_json(t)::jsonb), '[]'::jsonb)
  INTO _rows
  FROM (
    SELECT *
    FROM _pulse_admin_filtered f
    ORDER BY
